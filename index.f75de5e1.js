...
container.addEventListener("click", (e)=>{
    if (e.target.tagName === "BUTTON") {
        ...
        switch(action){
            case "append-row":
                if (rowsQuantaty >= 10) return;
                changeTable("add", "row");
                break;
            case "remove-row":
                if (rowsQuantaty <= 2) return;
                changeTable("remove", "row");
                break;
            case "append-column":
                if (cellsQuantaty >= 10) return;
                changeTable("add", "column");
                break;
            case "remove-column":
                if (cellsQuantaty <= 2) return;
    ...
    switch(type){
        ...
        case "column":
            if (todo === "add") {
                ...
                for(let k = 0; k < rowsQuantaty; k++)table.rows[k].cells[cellsQuantaty - 1].remove();
                cellsQuantaty--;
            }
            break;
    }
